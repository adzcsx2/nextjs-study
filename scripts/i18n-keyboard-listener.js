...
const PATH_CONFIG = (() => {
  ...
  return {
    ...
    // i18n 路径结构
    i18nDir: i18nPath,
    i18nLangDir: langPath,
    i18nZhDir: path.join(langPath, "zh"),
    i18nEnDir: path.join(langPath, "en"),

    // 翻译文件路径
    commonZhFile: path.join(langPath, "zh", "common.ts"),
    commonEnFile: path.join(langPath, "en", "common.ts"),

    // 缓存和临时文件
    cacheDir: path.join(baseRoot, ".i18n-cache"),
    ...
    i18n: {
      ...
      zhDir: "zh",
      enDir: "en",
      commonFile: "common.ts",
      networkFile: "network.ts",
    },
  };
})();
...
const PathUtils = {
  ...
  getRelativePath: (filePath) =>
    path.relative(PATH_CONFIG.projectRoot, filePath),

  /**
   * 获取相对于当前工作目录的路径
   */
  getRelativeToCurrentDir: (filePath) => path.relative(process.cwd(), filePath),

  /**
   * 检查文件是否在 i18n 目录下
   */
  isInI18nDir: (filePath) => filePath.includes(path.sep + "i18n" + path.sep),
  ...
  isInZhDir: (filePath) =>
    filePath.includes(
      path.sep + "i18n" + path.sep + "lang" + path.sep + "zh" + path.sep
    ),
  ...
  isSupportedExtension: (filePath) =>
    PATH_CONFIG.supportedExtensions.includes(path.extname(filePath)),

  /**
   * 检查目录是否应该被排除
   */
  shouldExcludeDir: (dirName) => PATH_CONFIG.excludedDirs.includes(dirName),

  /**
   * 生成缓存文件路径
   */
  getCacheFilePath: (filename) =>
    path.join(PATH_CONFIG.cacheDir, `${filename}.json`),

  /**
   * 生成中文翻译文件路径
   */
  getZhFilePath: (filename) => path.join(PATH_CONFIG.i18nZhDir, filename),

  /**
   * 生成英文翻译文件路径
   */
  getEnFilePath: (filename) => path.join(PATH_CONFIG.i18nEnDir, filename),

  /**
   * 生成环境变量文件路径
   */
  getEnvFilePath: (envFile) => path.join(PATH_CONFIG.projectRoot, envFile),

  /**
   * 获取中文通用翻译文件路径
   */
  getCommonZhPath: () => PATH_CONFIG.commonZhFile,

  /**
   * 获取英文通用翻译文件路径
   */
  getCommonEnPath: () => PATH_CONFIG.commonEnFile,
};
...
class AutoFileProcessor {
  ...
  async checkAndProcessFiles() {
    try {
      ...
      changedFiles.forEach((file) => {
        console.log(`  📄 ${PathUtils.getRelativeToCurrentDir(file)}`);
      });
  ...
  findRecentlyChangedFiles() {
    ...
    if (!fs.existsSync(PATH_CONFIG.srcDir)) {
      return changedFiles;
    }
      ...
      try {
        ...
        for (const item of items) {
          ...
          if (stat.isDirectory()) {
            ...
            // 只处理相关文件类型
            if (PathUtils.isSupportedExtension(item)) {
              // 检查文件是否在最近10秒内被修改
    ...
    searchFiles(PATH_CONFIG.srcDir);
    return changedFiles;
  }
  ...
  async processFileIfNeeded(filePath) {
    try {
      // 检查是否是中文翻译文件
      const isZhTranslationFile =
        PathUtils.isInZhDir(filePath) && path.extname(filePath) === ".ts";

      // 对于非翻译文件，检查是否应该处理
      if (!isZhTranslationFile && !this.processor.shouldProcessFile(filePath)) {
        console.log(
          `⚠️ 跳过不需要处理的文件: ${PathUtils.getRelativeToCurrentDir(
            filePath
          )}`
        );
        return;
      ...
      console.log(
        `\n🔄 处理文件: ${PathUtils.getRelativeToCurrentDir(filePath)}`
      );
      ...
      if (isZhTranslationFile) {
        ...
        // 使用 I18nSyncer 同步到英文文件
        try {
          const syncer = new I18nSyncer();
          await syncer.syncFile(filename);
      ...
      // 替换中文字符串
      content = this.processor.replaceChineseStrings(content, chineseStrings);
...
function loadEnvVariables() {
  for (const envFile of PATH_CONFIG.envFiles) {
    const envPath = PathUtils.getEnvFilePath(envFile);
        ...
        res.on("end", () => {
          try {
            ...
            if (result.Response && result.Response.Error) {
              reject(
                new Error(`腾讯翻译 API 错误: ${result.Response.Error.Message}`)
              );
...
class I18nSyncer {
  constructor() {
    ...
    this.cacheDir = PATH_CONFIG.cacheDir;
    this.zhDir = PATH_CONFIG.i18nZhDir;
    this.enDir = PATH_CONFIG.i18nEnDir;
  ...
  async translateBatch(translationTasks) {
    ...
    translationResults.forEach((result, index) => {
      ...
      if (result.status === "fulfilled") {
        ...
        console.error(`❌ 翻译最终失败: ${task.text}`, result.reason.message);
...
`;
    ...
    fs.writeFileSync(cacheFilePath, JSON.stringify(zhContent, null, 2), "utf8");
    console.log(`💾 已更新缓存文件`);
...
class ChineseStringProcessor {
  constructor() {
    this.srcPath = PATH_CONFIG.srcDir;
    this.i18nPath = PATH_CONFIG.i18nDir;
    this.commonZhPath = PathUtils.getCommonZhPath();
    this.commonEnPath = PathUtils.getCommonEnPath();
  ...
  replaceChineseStrings(content, chineseStrings) {
    ...
    chineseStrings.forEach((chineseStr) => {
      ...
      newContent = newContent.replace(jsxAttrRegex, `$1{t("${chineseStr}")}`);
      ...
      newContent = newContent.replace(doubleQuoteRegex, `t("${chineseStr}")`);
      ...
      newContent = newContent.replace(singleQuoteRegex, `t("${chineseStr}")`);
    });
  ...
  ensureUseTranslationImport(content) {
    ...
    if (importIndex >= 0) {
      const importStatement = 'import { useTranslation } from "@/i18n/hooks";';
  ...
  extractExistingKeys(content) {
    ...
    try {
      ...
      console.warn(`⚠️ 解析对象失败，使用正则表达式备用方案: ${error.message}`);
    }
  ...
  updateCommonFile(strings) {
    try {
      ...
      for (const str of validStrings) {
        const keyExists = existingKeys.has(str) || existingKeys.has(`"${str}"`);
      ...
      if (newStrings.length > 0) {
        ...
        for (const str of newStrings) {
          ...
          if (translation && !translation.startsWith("[翻译失败:")) {
            ...
            // 如果翻译失败，尝试本地词典翻译
            const localTranslation = syncer.translator.localTranslate(str);
            if (
              localTranslation &&
              !localTranslation.startsWith("[翻译失败:")
            ) {
              newEntries.push(`   "${str}": "${localTranslation}",`);
              console.log(`🔄 使用本地翻译: "${str}" → "${localTranslation}"`);
            } else {
              newEntries.push(`   "${str}": "${str}",`);
...
class KeyboardListener {
    ...
    rl.on("line", async (input) => {
      ...
      if (trimmedInput) {
        ...
        if (fs.existsSync(fullPath)) {
          ...
          console.log(
            `✅ 当前文件已手动设置为: ${path.relative(process.cwd(), fullPath)}`
          );
  ...
  async tryGetCurrentEditingFile() {
    try {
      ...
      const srcDir = PATH_CONFIG.srcDir;
    ...
    this.fileMonitoringInterval = setInterval(() => {
      ...
      if (fs.existsSync(srcDir)) {
        ...
        if (recentFile && recentFile !== this.currentFile) {
          this.currentFile = recentFile;
          console.log(
            `\n📝 检测到正在编辑: ${PathUtils.getRelativeToCurrentDir(
              recentFile
            )}`
          );
...
async function cleanupUnusedKeys() {
  const srcPath = PATH_CONFIG.srcDir;
  const zhCommonPath = PATH_CONFIG.commonZhFile;
  const enCommonPath = PATH_CONFIG.commonEnFile;
...
async function syncZhToEn() {
  try {
    const zhDir = PATH_CONFIG.i18nZhDir;
    const enDir = PATH_CONFIG.i18nEnDir;

    if (!fs.existsSync(zhDir)) {
      console.error(`❌ 中文目录不存在: ${zhDir}`);
      return;
    }

    if (!fs.existsSync(enDir)) {
      console.error(`❌ 英文目录不存在: ${enDir}`);
      return;
    }
    ...
    console.log(
      `� 发现 ${zhFiles.length} 个中文翻译文件: ${zhFiles.join(", ")}`
    );
